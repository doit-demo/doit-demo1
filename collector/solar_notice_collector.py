...
DEFAULT_SOURCES=[
 {"id":"google-solar","name":"Google News · 태양광 공고","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%ED%83%9C%EC%96%91%EA%B4%91%20%EA%B3%B5%EA%B3%A0&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-renewable","name":"Google News · 신재생에너지 공고","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EC%8B%A0%EC%9E%AC%EC%83%9D%EC%97%90%EB%84%88%EC%A7%80%20%EA%B3%B5%EA%B3%A0&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-agri","name":"Google News · 영농형태양광","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EC%98%81%EB%86%8D%ED%98%95%ED%83%9C%EC%96%91%EA%B4%91&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-sun-income","name":"Google News · 햇빛소득","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%ED%96%87%EB%B9%9B%EC%86%8C%EB%93%9D%20%EA%B3%B5%EA%B3%A0&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-development","name":"Google News · 발전사업 공고","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EB%B0%9C%EC%A0%84%EC%82%AC%EC%97%85%20%EA%B3%B5%EA%B3%A0%20%ED%83%9C%EC%96%91%EA%B4%91&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-public-land","name":"Google News · 공유재산 태양광","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EA%B3%B5%EC%9C%A0%EC%9E%AC%EC%82%B0%20%ED%83%9C%EC%96%91%EA%B4%91&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-community","name":"Google News · 주민참여 태양광","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EC%A3%BC%EB%AF%BC%EC%B0%B8%EC%97%AC%20%ED%83%9C%EC%96%91%EA%B4%91&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-rooftop","name":"Google News · 지붕형 태양광","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EC%A7%80%EB%B6%95%ED%98%95%20%ED%83%9C%EC%96%91%EA%B4%91&hl=ko&gl=KR&ceid=KR:ko"},
 {"id":"google-public-site","name":"Google News · 공공부지 태양광","kind":"rss","enabled":True,"url":"https://news.google.com/rss/search?q=%EA%B3%B5%EA%B3%B5%EB%B6%80%EC%A7%80%20%ED%83%9C%EC%96%91%EA%B4%91&hl=ko&gl=KR&ceid=KR:ko"},
]
CONFIG=Path("collector/sources.json");BIZINFO_URL="https://www.bizinfo.go.kr/uss/rss/bizinfoApi.do";OUT=Path("data/notices.json");STATUS=Path("monitor-status.json");MAX_ROWS=3000

def load_sources():
    try:
        obj=json.loads(CONFIG.read_text(encoding="utf-8")) if CONFIG.exists() else {}
        rows=obj.get("sources",[]) if isinstance(obj,dict) else []
        return [x for x in rows if isinstance(x,dict) and x.get("enabled",True) and x.get("url")]
    except Exception:
        return DEFAULT_SOURCES

def fetch(url,timeout=30):
    req=Request(url,headers={"User-Agent":"Mozilla/5.0 DSOLAR-Notice-Monitor/2.2"})
...
def main():
    ...
    by_key={str(key_for(x)):x for x in existing if isinstance(x,dict)};errors=[];statuses=[];collected=0;sources=load_sources()
    for source in sources:
        checked=datetime.now(timezone.utc).isoformat()
        try:
            rows=parse_xml_items(fetch(source["url"]),source["name"],source["url"])
            for row in rows:by_key[str(key_for(row))]=row
            collected+=len(rows);statuses.append({"name":source["name"],"id":source.get("id"),"status":"ok","items":len(rows),"checked_at":checked})
        except Exception as e:
            errors.append({"source":source["name"],"error":str(e)});statuses.append({"name":source["name"],"id":source.get("id"),"status":"error","items":0,"error":str(e),"checked_at":checked})
        time.sleep(.35)
    try:
