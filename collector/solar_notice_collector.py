...
KEYWORDS={"태양광":30,"신재생에너지":18,"영농형태양광":30,"햇빛소득":30,"발전사업":16,"공유재산":14,"유휴부지":14,"에너지":8,"RE100":10,"REC":6,"RPS":6,"농촌":8,"임대":6,"태양광발전":25,"태양광발전소":28,"태양광설치":22,"주민참여형":15,"수상태양광":22,"지붕형태양광":22,"염해농지":20,"유휴부지":14}
...
def fetch(url,timeout=30):
 req=Request(url,headers={"User-Agent":"Mozilla/5.0 DSOLAR-Notice-Monitor/2.0"})
...
def tag_score(text):
 ...
 if "태양광" in matched and any(k in matched for k in ("발전사업","영농형태양광","햇빛소득","공유재산","유휴부지","주민참여형","지붕형태양광")):score=min(100,score+10)
...
def parse_xml_items(xml_text,source_name,source_url):
 ...
 for item in root.iter():
  ...
  rows.append({"title":title,"description":desc[:900],"url":link,"source":source_name,"published":pub,"organization":org,"category":classify(title,desc),"score":score,"matched_keywords":matched,"collected_at":datetime.now(timezone.utc).isoformat()})
 return rows
...
def fetch_bizinfo():
 ...
 for x in candidates:
  ...
  rows.append({"title":title,"description":clean(str(desc))[:900],"url":url,"source":"기업마당 지원사업 API","published":str(pub),"organization":org,"region":str(region),"category":classify(title,str(desc)),"score":score,"matched_keywords":matched,"collected_at":datetime.now(timezone.utc).isoformat()})
